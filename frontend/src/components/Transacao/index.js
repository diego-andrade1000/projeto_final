...
const Transacao = () => {
  ...
  const [nomeDistribuidorTransacao, setNomeDistribuidorTransacao] =
    useState("");
  ...
  const cadastrarTransacao = () => {
    ...
    api.post("/api/transacao", transacao).then(() => {
        setCompraOuVendaTransacao(false);
        enqueueSnackbar('Transação feita com sucesso', {
            variant: "success",
            anchorOrigin: {
              vertical: "top",
              horizontal: "center",
  ...
  const editarTransacao = (id) => {
    ...
    api.put(`/api/transacao/${id}`, transacao).catch((err) => {
  ...
  const deletar = (id) => {
    api.delete(`/api/transacao/${id}`).catch((err) => {
  ...
  return (
    <>
      <Row className="linha">
        ...
        <Col md={2}>
          <Button variant="primary" onClick={() => getAllTransacoes()}>
            Atualizar
          </Button>
        </Col>
      </Row>
          ...
          {transacoes.length ? (
            transacoes.map((element) => (
              <>
                <tr>
                  <td>{element.id}</td>
                  {element.compraOuVenda ? <td> venda </td> : <td> compra </td>}
              ...
              <Form>
                <Row>
                  ...
                  <Col>
                    <Form.Check
                      ...
                      type="switch"
                      id="compraOuVenda"
                      value={compraOuVendaTransacao}
                      onChange={() => {
                        setCompraOuVendaTransacao(!compraOuVendaTransacao);
                        console.log(compraOuVendaTransacao);
                      }}
                    />
                  </Col>
                ...
                <Form.Group className="mb-3" controlId="nomeDoProduto">
                  ...
                  <Form.Select
                    ...
                    value={nomeProdutoTransacao}
                    onChange={(e) => setNomeProdutoTransacao(e.target.value)}
                  >
                ...
                <Form.Group className="mb-3" controlId="nomeDoDistribuidor">
                  ...
                  <Form.Control
                    ...
                    value={nomeDistribuidorTransacao}
                    onChange={(e) =>
                      setNomeDistribuidorTransacao(e.target.value)
                    }
                    disabled={compraOuVendaTransacao}
                  />
                </Form.Group>
